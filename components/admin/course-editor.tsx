...
export function CourseEditor({ course }: CourseEditorProps) {
  ...
  const [editCourseData, setEditCourseData] = useState({
    ...
    isStrictNavigation: course.isStrictNavigation,
  });
          ...
          {isEditingCourse ? (
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2 py-2">
                <input
                  type="checkbox"
                  id="strictNav"
                  checked={editCourseData.isStrictNavigation}
                  onChange={(e) => setEditCourseData(prev => ({ ...prev, isStrictNavigation: e.target.checked }))}
                  className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                />
                <label htmlFor="strictNav" className="text-sm font-medium text-gray-700">
                  Navegación Estricta (Quizzes requieren 70% para avanzar)
                </label>
              </div>
            ...
            <div className="group relative">
              ...
              <div className="mt-2 flex items-center gap-4">
                ...
                <span className={`px-2 py-1 rounded text-xs font-bold ${course.isStrictNavigation ? 'bg-indigo-100 text-indigo-800' : 'bg-slate-100 text-slate-800'
                  }`}>
                  {course.isStrictNavigation ? 'NAVEGACIÓN ESTRICTA' : 'NAVEGACIÓN LIBRE'}
                </span>
              </div>
            </div>
          )}
